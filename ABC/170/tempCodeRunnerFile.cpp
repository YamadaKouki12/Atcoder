...
signed main(){
    ...
    rep(i,n){
        p[i].F=a[i]; p[i].S=true;
        if(i!=n-1) if(a[i]==a[i+1]) p[i].S=false;
        if(i==n-1 && a[i-1]==a[i]) p[i].S=false;
    }
    //rep(i,n)if(p[i].S)cout<<3941394;

    rep(i,n){
        if(p[i].S==true){
            for(int j=i+1; j<n; j++){
                if(p[j].F%p[i].F==0) p[j].S=false;
            }
        }
    }
    int res=0;
    rep(i,n)if(p[i].S)res++;
    cout<<res<<endl;
