...
signed main(){
    ...
    range(i,1,n){
        if(s[i]=='A') wa[i][0]=wa[i-1][0]+1;
        else if(s[i]=='T') wa[i][1]=wa[i-1][1]+1;
        else if(s[i]=='C') wa[i][2]=wa[i-1][2]+1;
        else wa[i][3]=wa[i-1][3]+1;
    }
    rep(i,n-1){
        for(int j=i+1; j<n; j++){
            if(wa[j][0]-wa[i][0]==wa[j][1]-wa[i][1] && wa[j][2]-wa[i][2]==wa[j][3]-wa[i][3])
                res++;
        }
    }
    cout<<res<<endl;
