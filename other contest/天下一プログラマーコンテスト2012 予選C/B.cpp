...
signed main(){
    ...
    set<string> S,H,D,C;
    rep(i,n-1){
        if(s[i]=='S') S.insert(s.substr(i,2));
        else if(s[i]=='H') H.insert(s.substr(i,2));
        else if(s[i]=='D') D.insert(s.substr(i,2));
        else if(s[i]=='C') C.insert(s.substr(i,2));
        else{
            trash.pb(s[i]);
            trash.pb(s[i+1]);
        }
        if(S.size()==5 || H.size()==5 || D.size()==5 || C.size()==5) break;
    }
    cout<<trash<<endl;
